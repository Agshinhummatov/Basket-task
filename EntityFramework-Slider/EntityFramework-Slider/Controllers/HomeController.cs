...
namespace EntityFramework_Slider.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task <IActionResult> AddBasket(int? id)
        {
            ...
            //{
            //    Product productModel = await _context.Products.Where(x => !x.SoftDelete).Include(x => x.Images).Include(x => x.Category).FirstOrDefaultAsync(x => x.Id == item.Id);

            //    productsVm.Add(productModel);
            //}

            ////return RedirectToAction(nameof(Index));

            // return PartialView("_ProductsPartial", productsVm);
